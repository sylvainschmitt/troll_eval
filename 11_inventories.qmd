...
Inventory data from Paracou, including understorey trees, and Tapajos were used both as input to the TROLL assessment for the species list and as output to assess forest structure, neutral composition and species list.

```{r data}
#| eval: false
tapajos <- read_csv("data/tapajos/data/Tapajos_inventory_data_2010.csv", na = "-9999") %>% 
  filter(type == "PF") %>% 
...
paracou <- read_tsv("data/guyafor/guyafor.tsv") %>% 
...
paracou_understory <- read_csv("data/guyafor/understory_paracou_9ha_20231119.csv") %>% 
  mutate(plot = subplot) %>% 
