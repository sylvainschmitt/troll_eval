...
folderout1 <- snakemake@output[[1]]
folderout2 <- snakemake@output[[2]]
...
spinup <-  load_output(name = paste0(site, "_", cra, "_", crberr, "_", m),
                       path = folderin)
...
run2 <- troll(
  name = name,
  path = gsub(name, "", folderout2),
  global = parameters,
  species = run1@inputs$species, 
  climate = clim,
  daily = day,
  pedology = run1@inputs$pedology, 
  forest = get_forest(run1),
  soil = get_soil(run1), 
  load = FALSE,
  verbose = verbose,
  overwrite = TRUE
