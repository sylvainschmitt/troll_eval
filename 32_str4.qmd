...
bind_rows(
  best_par %>% 
    ...
    mutate(metric = "RRMSEP ALL", value = all_rmsep_norm),
  best_par %>% 
    slice_min(rmsep_dist) %>% 
    mutate(metric = "RMSEP Distribution", value = rmsep_dist),
  best_par %>% 
    slice_min(rmsep_ba) %>% 
    mutate(metric = "RMSEP BA", value = rmsep_ba),
  best_par %>% 
    slice_min(rmsep_abund) %>% 
    mutate(metric = "RMSEP Abundance", value = rmsep_abund),
  best_tap %>% 
    slice_min(all_rmsep_norm) %>% 
    mutate(metric = "RRMSEP ALL", value = all_rmsep_norm),
  best_tap %>% 
  ...
  best_tap %>% 
    slice_min(rmsep_agb) %>% 
    mutate(metric = "RMSEP AGB", value = rmsep_agb),
  best_tap %>% 
    slice_min(rmsep_abund) %>% 
    mutate(metric = "RMSEP Abundance", value = rmsep_abund)
  ) %>% 
  bind_rows() %>% 
  select(site, metric, value, a, b, m) %>% 
  kable()
```
