...
-   $b_{CR}$ : crown radius slope calibrated to 0.36 at Nouragues
...
pars <- data.frame(parameter = c("a", "error_b", "m"),
                   low = c(1.8, -4*0.08, 0.01),
                   high = c(2.5, 4*0.08, 0.03),
                   by = c(0.05, 0.05, 0.002))
...
read_tsv("outputs/calib_structure.tsv") %>% 
  ...
  gather(variable, value, -a, -b, -m, -site) %>% 
  ggplot(aes(a, value, col = b)) +
  geom_point() +
  facet_grid(variable ~ site, scales = "free_y") +
  theme_bw() +
  geom_hline(aes(yintercept = value),
             data = data.frame(site = c("Tapajos", "Tapajos", 
                                        "Paracou", "Paracou"),
                               value = c(470, 143.7, 612, 209.5), 
