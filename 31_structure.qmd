...
cowplot::plot_grid(
 dat %>% 
  ...
  xlab(expression(a[CR|m==0.024])) + ylab(expression(b[CR|m==0.024])),
   dat %>% 
...
read_tsv("outputs/calib_structure.tsv") %>% 
  ...
  scale_y_log10(labels = scales::comma) +
  xlab("DBH [ cm ]") + ylab(expression(Abundance~"["~ha^{-~1}~"]")) +
  scale_color_viridis_c(expression(a[CR|m==0.024|berr==0.02])) +
  theme_bw() +
...
read_tsv("outputs/calib_structure.tsv") %>% 
  ...
  xlab("DBH [ cm ]") + ylab(expression(Abundance~"["~ha^{-~1}~"]")) +
  scale_color_viridis_c(expression(b[CR])) +
  scale_color_viridis_c(expression(b[CR|m==0.024|a==2])) +
  theme_bw() +
  facet_wrap(~ site, nrow = 2)
```
...
read_tsv("outputs/calib_structure.tsv") %>% 
  ...
  group_by(site, a, b, m) %>% 
  summarise(abundance = sum(abundance), agb = sum(agb)/10^3) %>% 
