  ...
  geom_text(aes(col = "observed", label = lab), 
            data = data.frame(dbh_class = 5, 
                              ...
                              lab = c("3,787 trees/ha",
                                      "3.7 m2/ha"),
                              varlong = c("Log[10]~abundance~\"[\"~ha^{-~1}~\"]\"",
                                          "Basal~area~\"[\"~m^{2}~ha^{-~1}~\"]\""))) +
...
read_tsv("outputs/evaluation_forest2.tsv") %>% 
  ...
  geom_abline() +
  geom_linerange(aes(xmin = obs_l, xmax = obs_h), alpha = .2) +
  geom_linerange(aes(ymin = sim_l, ymax = sim_h), alpha = .2) +
  geom_point() + 
  theme_bw() +
  scale_y_log10() +
  scale_x_log10() +
...
t %>% 
  filter(trait %in% c("wsg", "LMA", "hmax")) %>% 
