...
```{r}
comp_sim <- read_tsv("outputs/evaluation_forest2.tsv") %>% 
  filter(dbh >= 10) %>% 
  mutate(plot = repetition) %>% 
  mutate(species = gsub("_", " ", s_name)) %>%
  ...
  summarise(l_sim = quantile(value, 0.025, na.rm = TRUE), 
            m_sim = quantile(value, 0.5, na.rm = TRUE), 
            h_sim = quantile(value, 0.975, na.rm = TRUE))
comp_obs <- read_tsv("outputs/composition.tsv") %>% 
  separate(species, "species") %>% 
  filter(variable == "abundance") %>% 
  group_by(site, species) %>% 
  summarise(l_obs = quantile(observed, 0.025, na.rm = TRUE), 
            m_obs = quantile(observed, 0.5, na.rm = TRUE), 
            h_obs = quantile(observed, 0.975, na.rm = TRUE))
g <- comp_sim %>% 
  left_join(comp_obs) %>% 
  ggplot(aes(m_sim, m_obs)) +
  geom_point() + 
  theme_bw() +
  ...
  scale_x_log10() +
  facet_wrap(~ site) +
  ylab(expression("Observed mean abundance ["~ha^{-~1}~"]")) +
  xlab(expression("Simulated mean abundance ["~ha^{-~1}~"]"))
ggsave(filename = "tests/abundance.png", plot = g, width = 7, height = 5)
```
...
troll_traits <- list(
  ...
  summarise(trait_value = median(trait_value))
g <- comp_sim %>% 
  left_join(comp_obs) %>% 
  select(site, species, m_sim, m_obs) %>% 
  gather(type, abundance, -site, -species) %>% 
  left_join(troll_traits) %>% 
  mutate(trait_long = recode(trait, "
                             ah" = "a[h]", 
                             "dbhmax" = "dbh[thres]~'['~m~']'", 
                             "hmax" = "h[lim]~'['~m~']'",
                             "leafarea" = "LA~'['~cm^2~']'",
                             ...
                             "tlp" = "pi[~tlp]~'['~MPa~']'", 
                             "wsg" = "WSG~'['~g~cm^{-~3}~']'")) %>% 
  ggplot(aes(trait_value, abundance, col = type)) +
  geom_point(alpha = .2) +
  theme_bw() +
  facet_wrap(~ trait_long, scales = "free_x", label = label_parsed) +
  scale_y_log10() +
  geom_smooth(method = "lm", se = FALSE) +
  ggpubr::stat_cor() +
  theme(legend.position = "bottom") +
  ...
  scale_color_manual("", values = as.vector(cols[c("obs", "sim")]), 
                     labels = c("Field", "TROLL"))
ggsave(filename = "tests/trait.png", plot = g)
```
